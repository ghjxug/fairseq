...
class LanguageClassificationCrossEntropyCriterion(
    ...
    def forward(self, model, sample, reduce=True, classification_step=False, language_classifier_one_vs_rest=0, vocab_size=256000):
        ...
        if classification_step:
            ...
            classifier_loss, classifier_nll_loss, n_correct, total, stats_per_lang = self.compute_encoder_classification_loss(sample["net_input"],
                                                                                                                              net_output,
            ...
            #     logging_output["n_correct"] = utils.item(n_correct.data)
            #     logging_output["total"] = utils.item(total.data)
            classifier_loss, classifier_nll_loss, _, _, _ = self.compute_encoder_classification_loss(sample["net_input"],
                                                                                                     net_output,
